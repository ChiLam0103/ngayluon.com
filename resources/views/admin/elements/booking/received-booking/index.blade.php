    ...
    <div class="row">
        ...
        <div class="col-lg-12">
            @include('admin.table_paging', [
               'id' => 'receive_booking',
               'title' => [
                       'caption' => 'Dữ liệu đơn hàng chưa giao',
                       'icon' => 'fa fa-table',
...
@push('script')
    <script>
        ...
        function loadListBoook(type_assign) {
            $.ajax({
                type: "GET",
                url: "{{ route('get_quick_assign_receive') }}",
                data: {
                ...
                if (msg.books.length > 0) {
                    $( msg.books ).each(function( index, value ) {
                        ...
                        bookLi += '<td>' + value.receive_name + '</td>';
                        bookLi += '<td>' + value.receive_phone + '</td>';
                        bookLi += '<td>' + value.receive_full_address + '</td>';
                        if (type_assign != 'no_assign') {
        ...
        $(document).ready(function(){
            ...
            $('#save-quick-assign').click(function(e){
                $.ajax({
                    type: "POST",
                    url: "{{ route('post_quick_assign_receive') }}",
                    data: {
